...
class SignupFreeViral extends React.Component {
    ...
    // TODO :: Sign In put rdir to SIGN IN 
    //http://localhost:3000/createaccount?firstname=gaious testing&lastname=gaioustesting&email=newemail@email.com&rdir=http%3A%2F%2Flocalhost%3A3000%2Fshare-funnel%3Fshared%3Dtrue%26token%3DNWJmZWIyMWI0OWFjZGEwZmI3ZTQwMGVk%26name%3DZnVubmVsLXNoYXJlZA%3D%3D%26fid%3DNWY2OGQ4NTk0NzMxMTllYzJhZTEzMGIx
    /*
     https://app.productlistgenie.io/createaccount?email=jose@email.com&rdir=https%3A%2F%2Fapp.productlistgenie.io%2Fshare-funnel%3Fshared%3Dtrue%26token%3DNWMyN2VjMjU0MjUwNzU3ZTU1YTJkMTAx%26name%3Db2FzaXNwb29s%26fid%3DNWY0ZWIyZjFjNTRkNWYwNDQ4ZTQwNjY2
    */
            ...
            if (values.rdir) {
                if (user === null) {
                    if (values.email) {
                        ...
                        // isUserExist => loginForm Password Only
                    ...
                    const newURI = new URL(values.rdir);

                    let businessname = values.firstname + values.lastname;
                    // console.log(`rdir=`+encodeURIComponent("https://app.productlistgenie.io/share-funnel?shared=true&token=NWMyN2VjMjU0MjUwNzU3ZTU1YTJkMTAx&name=b2FzaXNwb29s&fid=NWY0ZWIyZjFjNTRkNWYwNDQ4ZTQwNjY2"));
                    this.props.history.push(`${newURI.pathname}/?shared=${newURI.searchParams.get('shared')}&token=${newURI.searchParams.get('token')}&name=${newURI.searchParams.get('name')}&fid=${newURI.searchParams.get('fid')}&businessname=${businessname.toLowerCase()}&fromViral=true`);
                }
            }
        } catch (err) {
            console.log(err);
        }
        // TODO :: at the first Load
    ...
    handleSubmit(event, signupUser) {
        ...
        signupUser().then(async ({ data }) => {
            ...
            if (this.state.rdir) {
                // * Redirect to new Link 
                console.log(this.state.rdir);
                const newURI = new URL(this.state.rdir);
                let businessname = this.state.firstName + this.state.lastName;
                

                this.props.history.push(`${newURI.pathname}/?shared=${newURI.searchParams.get('shared')}&token=${newURI.searchParams.get('token')}&name=${newURI.searchParams.get('name')}&fid=${newURI.searchParams.get('fid')}&fromViral=true&businessname=${businessname.toLowerCase()}&fromViral=true`);
    ...
    render() {
        ...
        return (
            <div className="center-vertical-parent funnel" style={{ height: '100vh' }}>
                ...
                `}} />
                {/* signup for the free viral to link */}
                <div className="center-vertical" style={{ height: '100%' }}>
                    <div className="row-separator">
                        <div className="column column_10_12 " style={{ backgroundColor: '#212a31', padding: 40, height: '100%' }} id="signup-form">
                            <div className="row-separator">
                                <label className="font-questrial-light color-green" style={{ fontSize: '2em' }}>Continue to share product.</label>
                            </div>
                                ...
                                {(signupUser, { data, loading, error }) => {
                                    return (
                                        <form className="signup-container" onSubmit={event => this.handleSubmit(event, signupUser)} style={{ caretColor: 'green' }}>
                                            ...
                                            <div className="row-separator">
                                                <input type="text" name="firstName" placeholder="First Name" value={firstName} onChange={this.handleChange.bind(this)} readOnly={true} />
                                            </div>
                                            <div className="row-separator">
                                                <input type="text" name="lastName" placeholder="Last Name" value={lastName} onChange={this.handleChange.bind(this)} readOnly={true} />
                                            </div>
                                            <div className="row-separator">
                                                <input type="email" name="email" placeholder="Email" value={email} onChange={this.handleChange.bind(this)} readOnly={true} />
                                            </div>
                                            <div className="row-separator">
...
export default withRouter(SignupFreeViral);