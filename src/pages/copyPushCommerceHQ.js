...
class CopyPushCommerce extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            disableBtn: false,
    ...
    pushpr(product_link, wh) {
        ...
        function push(self) {
            toastr.clear();
            toastr.info('CommerceHQ takes a while to clone, please wait...', 'Granting Your wish');
            var data = {
            ...
            fetch('/api/v3/commerceHQ', {
                ...
                .then(resultData => {
                    if (resultData.status === "success") {
                        ...
                        toastr.clear();
                        points.toastrPrompt(toastr, 'error', 'There\'s something wrong with the shop you entered. Please try again. ', 'Copy Push Commerce HQ');
                    }
                }).catch(err => {
                    toastr.clear();
                    points.toastrPrompt(toastr, 'error', 'Something went wrong. Please try again', 'Copy Push Commerce HQ');
                });
        }
    }
            ...
            <div>
                <Modal open={this.props.open} closeModal={!this.state.disableBtn ? this.props.closeModal : null}>
                    <div className="text-center">
                        <h2>Copy product for CommerceHQ from other shopify store</h2>
...
export default CopyPushCommerce;