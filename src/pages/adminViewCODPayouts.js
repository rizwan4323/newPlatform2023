...
class AdminViewCODPayouts extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            floc: 'ALL',
    ...
    head() {
        return (
            <Helmet bodyAttributes={{ class: "dashboardPage" }}>
                <title>All COD Payouts - Product List Genie</title>
            </Helmet>
        );
    }
                ...
                <div className="newPageHeader">
                    <div className="column column_12_12 display-inline flex-container" style={{ justifyContent: 'flex-start' }}>
                        <div className="row-separator" style={{ margin: '0 5px' }}>
                            ...
                            <h4 className="font-roboto-bold" style={{ fontSize: '1.5em', color: '#273037' }}>All COD Payouts</h4>
                ...
                <div className="page-container">
                    <div className="column column_12_12" style={{ padding: 0 }}>
                        <div className="filter-container">
                            <Link to="/admin-manage-cod-payouts" style={{ lineHeight: 0, marginRight: 10 }}>Manage Payout</Link>
                            <Checkbox
                                id="show_vip"
                                label="Show VIP"
                    ...
                    <div className="column column_9_12" style={{ padding: 0 }}>
                        <div className="product-card">
                            {state.payBy == "payout" &&
                                <Table headers={[{ text: "Email" }, { text: "Name" }, { text: "Delivered Payout" }, { text: "Collected Payout" }]} containerStyle={{ fontSize: '0.9em', padding: 10 }} >
                                                        ...
                                                        <div>
                                                            <Query query={GET_MY_PAY_CHECK}
                                                                ...
                                                                {({ data, loading, refetch, error }) => {
                                                                    ...
                                                                    return <label>${points.commafy(data.getMyPayCheck.count.toFixed(2))}</label>;
                                                                }}
                                                            </Query>
                                                        </div>
                                                        <div>
                                                            <Query query={GET_MY_PAY_CHECK}
                                                                ...
                                                                {({ data, loading, refetch, error }) => {
                                                                    ...
                                                                    return <label className="color-green">${points.commafy(data.getMyPayCheck.count.toFixed(2))}</label>;
                                                                }}
                                                            </Query>
                                                        </div>
                                                    </Tbody>
...
export default withAuth(session => session && session.getCurrentUser)(AdminViewCODPayouts);