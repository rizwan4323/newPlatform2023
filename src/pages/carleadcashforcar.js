...
export default class CashForCar extends React.Component {
    constructor(props) {
        super();
        this.state = {
            name: "",
        ...
        this.validateForm().then(res => {
            ...
            if (res) {
                ...
                const payload = {
                    'query': `
                        mutation {
                            addUserbehavior(sessionID: "${Date.now() + "PLG" + Date.now() + "CRL"}", creatorID: "5c27ec254250757e55a2d101", firstName: "${this.state.name}", email: "${this.state.email}", phone: "${this.state.phone.replaceAll("+1", "").replaceAll("-", "")}", notes: "${this.state.notes.replace(/"/gi, '\\\"')}", tags: "${"cashforcar"}", affiliateEmail: "${this.props.session.getCurrentUser.email}", affiliateID: "${this.props.session.getCurrentUser.id}", date: "${Date.now()}"){
                                sessionID
                                creatorID
                                firstName
                                email
                ...
                points.customFetch('https://stats.productlistgenie.io/graphql', "POST", payload, value => {
                    try {
                        ...
                        toastr.options.extendedTimeOut = 2000;
                        toastr.clear();
                        toastr.success("Added Cash for Cars Leads ", "Success!");                        
                        this.setState({
                            fromCards: false,
                            email: "",
                            name: "",
    ...
    head() {
        return (
            <Helmet>
                <title>Cash for Cars - Product List Genie</title>
            </Helmet>
        );
    }
