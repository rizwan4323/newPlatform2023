...
module.exports = {
	checkout: function (user_id, page_data, faddons) {
		return /* html */ `
        console.log("19.7. Initializing Braintree subscription upsell...");
        unlayer.registerTool({
            type: 'Braintree-subscriptionbump', // Todo: Change the name Aug5
            category: 'contents',
            ...
            options: {
                editor_title: {
                    ...
                    options: {
                        Braintree: {
                            "label": "Tittle",
                            "defaultValue": {
                                "title": "Braintree Subscription Order Bump",
                                "description": "Can be added along with Braintree's regular checkout as an add-on",
                            },
                            "widget": "editor_title"
                        }
                    }
                ...
                confirmation: {
                    title: "Apperance",
                    position: 2,
                    options: ${editorTools.config_order_bump_subscription("addonPrice,variants")}
                },
            ...
            renderer: {
                ...
                exporters: {
                    web: function(values) {
                        ...
                        <div class="product-addons">
                            <img class="checkbox-arrow-slide-right" src="https://cdn.productlistgenie.com/images/NWMwYTg1ZjA3NzMwMDEzMjdiMDM5MDRj/1560786344044256-256-7f4d535dc832b66d90d5c0cb72ec8690-arrow.png">
                            \$\{values.checked ? ' <input type="checkbox" class="p_add_on" id="braintree_order_bump_id" style="margin-left: 15px; position: relative;" checked  />' : '<input type="checkbox" class="p_add_on" id="braintree_order_bump_id" style="margin-left: 15px; position: relative;" />'\} 
                            <label style="margin-left: 5px; text-decoration: \$\{values.undeline ? 'underline' : 'unset'\};">\$\{values.addonText\}</label>
                        </div>

                            <div style="display:none"  id="braintree_order_bump"
                            ...
                            <script>
                                const subscription_orderBump = async (element) =>{
                                    ...
                                    if (localStorage.getItem("braintree_user_ids")) {
                                        ...
                                        payload.cog = cog ? cog : 0;
                                        payload.productPrice = parseFloat(price)
                                        payload.productName = product_name;
                                        payload.variantName = variant_name;
                                        payload.planID = braintree_subscription_id;
                                        payload.refToken = payload.tk;
                                        payload.env = localStorage.getItem("live");
                                        plgQS() !== "" ? payload.campaign_src = plgQS() : void 0;
                                        payload.variant_id = "\$\{values.shopify_varian_Id\}";
                                        
                                        const createSubscription = await fetch("/braintree-subscription-create-upsell", {
                                                                    method: "POST",
                                                                    headers: { 'Content-Type': 'application/json' },
                                                                    body: JSON.stringify(payload)
                                                                })

                                        const createSubscriptionJson = await createSubscription.json();

                                        if (createSubscriptionJson.success) {
                                            payload.result = createSubscriptionJson.subscription
                                            const response = await fetch('/braintree-orders-initial', {
                                                method: "POST",
                                                headers: {
                                            ...
                                            console.log("save_braintree_initial_data_subscription", data);
                                            return;
                                            } else {
                                                let message = createSubscriptionJson?.message ?? "Invalid Subscription Plan ID";
                                                alert(message);
                                            }
                                        console.log("createSubscriptionJson", createSubscriptionJson);
                                     return   
                                    } else {
