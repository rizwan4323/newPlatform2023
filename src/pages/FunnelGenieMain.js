...
class FunnelGenieMain extends React.Component {
    constructor(props) {
        super();
        this.state = {
    ...
    head() {
        return (
            <Helmet bodyAttributes={{ class: "dashboardPage" }}>
                <title>Funnel Genie Main - Product List Genie</title>
            </Helmet>
        );
    }
                                    ...
                                    <div className="column column_12_12">
                                        <div className="form_row" style={{ position: 'relative' }}>
                                            ...
                                            <span className="clickable fas fa-plus" style={{ position: 'absolute', top: 33, right: 5, background: '#ff7838', color: '#fff', padding: '6px 7px', borderRadius: 3 }} title="Add more Domain" onClick={() => {
                                                // TODO :: User New Privilede for User Number 0                                                 
                                                if (currentUser.privilege == 0 && currentUser.funnel_genie_domains.length >= 1) {
                                                    this.trialMaxDomainReach();
                                                }
                                                else if (currentUser.privilege == 1 && currentUser.funnel_genie_domains.length >= 2) { // User Privilege
                                                    this.trialMaxDomainReach();
                                                } else {
...
export default withAuth(session => session && session.getCurrentUser)(FunnelGenieMain);